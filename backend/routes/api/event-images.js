...
//delete image by eventimage id
router.delete('/:id', requireAuth, async (req, res, next) => {
    const theEventImage = await EventImage.findByPk(req.params.id)
    if (!theEventImage) {
    ...
    const theEventId = theEventImage.id
    ...
    const theMembers = await Membership.findAll({
        where: { groupId: theGroupId.groupId }
    })
